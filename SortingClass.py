class SortingClass:
    ...
    ###############################################################
    # Selection sort finds the smallest value in a list and moves it to the front. The next iteration will assume the
    # first item is sorted and try to find the smallest value in the sub-list and move that to the next available index.
    # Swaps continue until the list is sorted.
    ###############################################################
    ...
    # iterated through for sorting. The variable value is equal to the value at index element because it represents
    # the next item to be compared. The while loop compares each element in the sorted sub-list to value to find
    ...
    def bubble_sort(l):
        ...
        for e1 in range(len(l) - 1):
            ...
            # Iterates through len(l) - 1 - e1 because items should be sorted at the end of the list after iterations
    ...
    ###############################################################
    def rec_bubble_sort(self, l, length):
        if length == 1:
    ...
    # called and recursively separates the list into sub-lists. Once the base case is met the first call to merge() is
    # done, which returns a single item sub-list. This sub-list is merged with another single item sub-list and then
    # they are both passed to merge(), which will sort their values into a new sub-list and return that. This goes on
    ...
    def merge_sort(self, l):
        ...
        return self.merge(self.merge_sort(l[:mid]), self.merge_sort(l[mid:]))
